...
def get_X_ypre(config_path_list, path_y):
    ...
    tmp_ypre_train = np.array(pickle.load(open(tinyimagenet_densenet_prediction_train_coreml, 'rb')))
    tmp_ypre_test = np.array(pickle.load(open(tinyimagenet_densenet_prediction_test_coreml, 'rb')))
    imagenet_data = check(pickle.load(open(tiny_imagenet_data, 'rb')))

    tmp_x_train, tmp_x_test, y_train, y_test = train_test_split(imagenet_data, y, test_size=0.2, random_state=5)
    X_list_train.append(tmp_x_train)
    ypre_list_train.append(tmp_ypre_train)
    y_list_train.append(y_train)

    X_list_test.append(tmp_x_test)
    ypre_list_test.append(tmp_ypre_test)
    y_list_test.append(y_test)
...
save_flag = 'tinyimagenet_densenet_coreml'
path_tree_model = 'tinyimagenet_densenet_coreml.model'
path_graph_model = 'tinyimagenet_densenet_coreml_node2vec.pkl'

x_train, x_train_prediction, y_train, x_test, x_test_prediction, y_test = get_X_ypre(path_pre_data_densenet_coreml_data, tiny_imagenet_label)
