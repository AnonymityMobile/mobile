...
def run_node2vec():
    save_embedding = 'anything_node2vec.pkl'
    tree_path = 'anything_tree.txt'
...
def run_line():
    save_embedding = 'anything_line.pkl'
    tree_path = 'anything_tree.txt'
    G = nx.read_edgelist(tree_path, create_using=nx.DiGraph(), nodetype=None, data=[('weight', int)])
...
def run_deepwalk():
    save_embedding = 'anything_deepwalk.pkl'
    tree_path = 'anything_tree.txt'
    G = nx.read_edgelist(tree_path, create_using=nx.DiGraph(), nodetype=None, data=[('weight', int)])
...
def run_struc2vec():
    save_embedding = 'anything_struc2vec.pkl'
    tree_path = 'anything_tree.txt'
    G = nx.read_edgelist(tree_path, create_using=nx.DiGraph(), nodetype=None, data=[('weight', int)])
