...
def get_path_list_lenet1(path_dir_compile):
    path_list = []
    if os.path.isdir(path_dir_compile):
        for root, dirs, files in os.walk(path_dir_compile, topdown=True):
            for file in files:
                file_absolute_path = os.path.join(root, file)
                if 'enhancement_prediction_lenet1' in file_absolute_path:
                    path_list.append(file_absolute_path)
    return path_list

def get_path_list_lenet5(path_dir_compile):
    path_list = []
    if os.path.isdir(path_dir_compile):
        for root, dirs, files in os.walk(path_dir_compile, topdown=True):
            for file in files:
                file_absolute_path = os.path.join(root, file)
                if 'enhancement_prediction_lenet5' in file_absolute_path:
                    path_list.append(file_absolute_path)
    return path_list
...
path_list = get_path_list_lenet5('/mnt/irisgpfs/users/pycharm/data/mnist_pkl')
